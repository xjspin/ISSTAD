...
if __name__ == '__main__':
    ...
    test_set = AD_TEST(args, args.test_data_dir)
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        with torch.no_grad():
            ...
            for img, pn, tiff_path, classes in val_bar:
                ...
                pn_mean_list.append(1 - pn.item())
                if classes[0] == 'good':
                    pn_logic_list.append(1 - pn.item())
                    logic_list.append(np.mean(admap))
                    pn_struc_list.append(1 - pn.item())
                    struc_list.append(np.mean(admap))
                if classes[0] == 'logical_anomalies':
                    pn_logic_list.append(1 - pn.item())
                    logic_list.append(np.mean(admap))
                if classes[0] == 'structural_anomalies':
                    pn_struc_list.append(1 - pn.item())
                    struc_list.append(np.mean(admap))                                  
            ...
            print(
                "mean_auc", roc_auc_score(pn_mean_list, mean_list), 
                "logical_auc", roc_auc_score(pn_logic_list, logic_list), 
                "structural_auc", roc_auc_score(pn_struc_list, struc_list), file=f)
            f.close()


            if best_auc < roc_auc_score(pn_mean_list, mean_list):
                best_auc = roc_auc_score(pn_mean_list, mean_list)               
                f = open( auc_saved_path + 'result.txt','w')
                print(
                        "mean_auc", roc_auc_score(pn_mean_list, mean_list), 
                        "logical_auc", roc_auc_score(pn_logic_list, logic_list), 
                        "structural_auc", roc_auc_score(pn_struc_list, struc_list), file=f)
        ...
        #spro evaluation sub-thread
        spro_saved_path_ = spro_saved_path+str(epoch)+'/'
        python_script = './step2_pixel_ss_learning/evluation/mvtec_loco_ad_evaluation/evaluate_experiment.py' 
        process = subprocess.Popen(['python', python_script, '--object_name', object_name,
                                    '--dataset_base_dir', args.resize_data_dir ,
                                    '--anomaly_maps_dir', args.admap_saved_dir,
                                    '--output_dir', spro_saved_path_])
        process.wait()
